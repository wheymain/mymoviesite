...
cursor.execute('create table if not exists movie (id, title, original_title, url, rating, images, directors, casts, castsids, year, genres, countries, summary)')
...
def add_movie(data):
	...
	db.insert('movie', 
		...
		castsids=','.join([d['id'] for d in movie['casts']]),
		year=movie['year'],
...
for movie in top250:
	...
	print(movie['id'],movie['title'], movie['casts'])
